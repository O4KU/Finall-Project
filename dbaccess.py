...
def search_users(search, srch_type):
    ...
    if srch_type == "Customer":
        res = cur.execute(
            "SELECT custID, name, email, phone, city, zipcode, country FROM customer WHERE LOWER(name) like ?",
            (search,))
    elif srch_type == "Seller":
        res = cur.execute(
            "SELECT sellID, name, email, phone, city, zipcode, country FROM seller WHERE LOWER(name) like ?",
            (search,))
    res = [i for i in res]
    conn.close()
    return res
...
def update_details(data, userid, type):
    ...
    if type == "Customer":
        cur.execute(
            "UPDATE customer SET phone=?, city=?, zipcode=?, country=?,  where custID=?",
            (data["phone"],
             data["city"],
             data["zip"],
             data["country"],
        ...
        cur.execute(
            "UPDATE seller SET phone=?, area=?, locality=?, city=?, state=?, country=?, zipcode=? where sellID=?",
            (data["phone"],
             data["city"],
             data["zip"],
             data["country"],
